...
EAR_THRESHOLD = 0.3
BLINK_CONSEC_FRAMES = 2 #changed from 7 to 2------------------2
HEAD_DOWN_THRESHOLD = 0.4  # Relative position threshold for head-down detection
...
last_eye_closed_time = 2 # made changes--------------------1
...
# Set up containers for status display
status_container = st.empty()
blink_counter = st.empty()
head_down_counter = st.empty()

# Tkinter function to display system popup alerts
last_alert_time=0
ALERT_INTERVAL = 4
...
def show_alert(message):
    ...
    if current_time - last_alert_time > ALERT_INTERVAL:
        ...
        root.after(0,lambda: messagebox.showinfo("Alert", message))  # Show the alert message box

def update_popup_with_timer(start_time):
    elapsed_time = int(time.time() - start_time)
    message = f"Head down detected! Time: {elapsed_time}s"
    show_alert(message)

# Initialize the drowsiness message
drowsiness_message = "You are drowsy! Please stay alert"
...
while True:
    ...
    for i in range(len(boxes)):
        ...
        if label == 'Drowsy':
            ...
            drowsiness_message = "You are drowsy! Please stay alert."
            # Trigger a system alert for drowsiness
            show_alert("You are drowsy! Please stay alert.")
        elif not drowsiness_detected and head_down_start_time is None:
            drowsiness_message = "You are awake"
    ...
    if results_mesh.multi_face_landmarks:
        ...
        for face_landmarks in results_mesh.multi_face_landmarks:
            ...
            if head_pos_y > HEAD_DOWN_THRESHOLD:  # If the head position is below a threshold
                head_down_message = "Your head is down. Please raise your head."
        ...
        if head_down_start_time is None:
            ...
            update_popup_with_timer(head_down_start_time)  # Show the timer popup for the first time
    ...
    if head_down_start_time:
        ...
        if head_down_event_duration >= 5:  # Adjust threshold to suit your needs
            head_down_count += 1
            head_down_start_time = None  # Reset the head down timer after counting
    ...
    status_container.markdown(f"**{drowsiness_message}**")
    blink_counter.markdown(f"**Blink Count: {blink_count}**")
    head_down_counter.markdown(f"**Head Down Count: {head_down_count}**")
